$(document).ready(function(){
    ...
    $("#ac_login").click(function(){
        ...
        if(account_name.val()=="admin"){//这个页面不允许管理员登录
            alert("用户名非法");
            return;
        }
        ...
        $.ajax({
            url:`http://39.101.199.3:8080/myProj-1.0-SNAPSHOT/1.0/users/login/${auth_code.val()}/1`,
            type:"POST",
            cache:false,
    ...
    $("#em_login").click(function(){
        ...
        $.ajax({
            url:`http://39.101.199.3:8080/myProj-1.0-SNAPSHOT/1.0/users/login/${em_auth_code.val()}/1`,
            type:"POST",
            cache:false,
            data:JSON.stringify(data),
            dataType:"json",
